...
public class Quiz8Activity extends AppCompatActivity {
        ...
        bottomNavigationView.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(MenuItem item) {
                ...
                if (item.getItemId() == R.id.home) {
                    intent = new Intent(Quiz8Activity.this, MainActivity.class);
                    intent.putExtra("UserID" , UserID);
                    intent.putExtra("Username" , Username);
                    startActivity(intent);
                    return true;
                } else if (item.getItemId() == R.id.lessons) {
                    intent = new Intent(Quiz8Activity.this, MenuActivity.class);
                    intent.putExtra("UserID" , UserID);
                    intent.putExtra("Username" , Username);
                    startActivity(intent);
                    return true;
                } else if (item.getItemId() == R.id.exercise) {
                    intent = new Intent(Quiz8Activity.this, QuizActivity.class);
                    intent.putExtra("UserID" , UserID);
                    intent.putExtra("Username" , Username);
                    startActivity(intent);
                    return true;
                } else if (item.getItemId() == R.id.profile) {
                    intent = new Intent(Quiz8Activity.this, ProfileActivity.class);
                    intent.putExtra("UserID" , UserID);
                    intent.putExtra("Username" , Username);
                    startActivity(intent);
                    return true;
    ...
    private void checkAnswers() {
        if (radioGroup1.getCheckedRadioButtonId() == -1 ||
            ...
            boolean res = DB.insertScore(Integer.parseInt(UserID),8, score);
            if (res) {
                Toast.makeText(Quiz8Activity.this, "ได้คะแนนทั้งหมด : " + score + " คะแนน", Toast.LENGTH_LONG).show();
                Toast.makeText(Quiz8Activity.this, message, Toast.LENGTH_LONG).show();
            } else {
                Toast.makeText(Quiz8Activity.this, "เกิดข้อผิดพลาด", Toast.LENGTH_LONG).show();
            }
        }
    }
}
